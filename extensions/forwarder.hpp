...
private:
  ...
  //new fields to verify before cache
  std::vector<shared_ptr<const Data>> m_dataToBeVerified;//list of data that's awaiting verification... I'm hoping an interest pulls in multiple datas to the same node.
  const std::string m_keyPrefix = "/prefix/key"; //constant string for the key since this is a simulation... only 1 key in the system => would be better to have a map or custom data structure for key points to data list
  const uint32_t m_goodPayloadSize = 1024;
  uint32_t m_keyNonce = 255;
  bool m_keyOriginatedFromHere = false;
  std::vector<FaceId> m_dataToBeVerifiedInFaces;
